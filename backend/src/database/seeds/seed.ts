...
import { User, UserRole } from '../../modules/users/entities';
import { Blog, BlogStatus } from '../../modules/blogs/entities';
import { Slide, SlideVisualType } from '../../modules/slides/entities';
  ...
  try {
    ...
    if (isProduction) {
      const existingAdmin = await userRepository.findOne({
        where: { username: 'victor.olivares' },
      });
    ...
    // Obtener IDs de usuarios para blogs
    const adminUser = await userRepository.findOne({
      where: { username: 'admin' },
    });
    const victorUser = await userRepository.findOne({
      where: { username: 'victor.olivares' },
    });
    const editorUser = await userRepository.findOne({
      where: { username: 'editor.test' },
    });

    if (!adminUser || !victorUser || !editorUser) {
      console.error(
        'Error: No se encontraron todos los usuarios necesarios para crear blogs',
      );
      await AppDataSource.destroy();
      return;
    }
    ...
    const blogEntries = [
      {
        title:
          'Automatización de Nóminas: El Cambio que tu Departamento de RRHH Necesita',
        description:
          'Descubre cómo automatizar nóminas puede transformar tu departamento de RRHH. Guía completa sobre beneficios, implementación y mejores prácticas.',
      ...
      {
        title:
          'Cultura Organizacional Remote-First: Construyendo Conexión en la Distancia',
        description:
          'Herramientas, estrategias y mejores prácticas para fortalecer cultura empresarial en equipos 100% remotos o híbridos. Engagement real en la era del trabajo distribuido.',
...
En EMMA estamos construyendo las herramientas para que lo hagas bien.`,
        author_id: victorUser.id,
        slug: 'cultura-organizacional-remote-first',
        status: BlogStatus.PUBLISHED,
      },
      {
        title:
          'IA Predictiva: La Herramienta que Anticipará la Rotación de tu Talento Clave',
        description:
          'Descubre cómo la IA identifica señales tempranas de rotación. Retén talento clave con predicciones 94% precisas antes de que sea demasiado tarde.',
...
EMMA está aquí para ese futuro.`,
    ...
    for (const blogData of blogEntries) {
      ...
      if (!existingBlog) {
        ...
        console.log(`✅ Blog creado: ${blogData.slug}`);
      } else {
        console.log(`ℹ️  Blog ya existe: ${blogData.slug}`);
      }
    }
    ...
    const slides = [
      {
        ...
        description:
          'EMMA combina lo mejor de la tecnología Azure con 500+ horas de desarrollo dedicadas. Desde pequeñas startups hasta grandes corporaciones: nómina automática, evaluaciones avanzadas, workflows personalizables y comunicación unificada. 80% menos tiempo en tareas administrativas. ¡Únete a los pioneros!',
        button_text: 'Solicitar Demo',
        button_link: '/contact',
        visual_type: SlideVisualType.DASHBOARD,
        is_active: true,
        sort_order: 1,
      },
      {
        ...
        description:
          'Nómina automática, evaluaciones 360°, onboarding inteligente y más. EMMA consolida todos tus procesos de RRHH en una plataforma segura, escalable y fácil de usar. 10K+ empleados soportados. 99.9% de disponibilidad en Azure. ¡Comienza hoy!',
        button_text: 'Ver Funcionalidades',
        button_link: '/about',
        visual_type: SlideVisualType.ANALYTICS,
        is_active: true,
        sort_order: 2,
      },
      {
        ...
        description:
          'Gestiona nóminas, beneficios, capacitación y cultura en un solo lugar. EMMA está diseñado con estándares empresariales de seguridad, comunicación unificada en tiempo real y herramientas para fomentar el crecimiento del talento. Sé pionero en la transformación digital de tu empresa.',
        button_text: 'Comenzar Ahora',
        button_link: '/contact',
        visual_type: SlideVisualType.INNOVATION,
        is_active: true,
        sort_order: 3,
      },
      {
        ...
        description:
          'EMMA integra administración, nómina, evaluaciones, reconocimiento y beneficios flexibles. Reduce procesos repetitivos, mejora el engagement con herramientas de clima organizacional y peer-to-peer, y escala sin límites. Infraestructura Azure para máxima confiabilidad.',
        button_text: 'Conoce más',
        button_link: '/about',
        visual_type: SlideVisualType.TEAM,
        is_active: true,
        sort_order: 4,
      },
      {
        ...
        description:
          'Olvida las hojas de cálculo. EMMA automatiza nóminas, gestiona beneficios, facilita onboarding y fomenta una cultura organizacional sólida. Con soporte 24/7, seguridad empresarial y capacidad para 10K+ empleados, EMMA es la solución completa que estabas buscando.',
        button_text: 'Únete al Beta',
        button_link: '/contact',
        visual_type: SlideVisualType.GROWTH,
        is_active: false,
        sort_order: 5,
      },
    ];
    ...
    for (const slideData of slides) {
      const existingSlide = await slideRepository.findOne({
        where: { sort_order: slideData.sort_order },
      });

      if (!existingSlide) {
        const slide = slideRepository.create(slideData);
        await slideRepository.save(slide);
        console.log(`✅ Slide creado: ${slideData.title}`);
      } else {
        console.log(`ℹ️  Slide ya existe: ${slideData.title}`);
      }
    }
