...
export default function RecruitmentsPage() {
  ...
  const fetchFiles = async (recruitmentId: number) => {
    try {
      const related_type = "recruitments";
  ...
  const fetchJobPositions = async () => {
    try {
      const response = await fetch(`/api/job-positions?page=1&limit=100`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const uploadFile = async (recruitmentId: number, file: File) => {
    try {
      ...
      formData.append("related_type", "recruitments");
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "No se pudo subir el archivo del candidato";
  ...
  const openDialog = (recruitment?: Recruitment) => {
    ...
    if (recruitment) {
      setForm({
        ...
        salary_expectation: recruitment.salary_expectation ?? "",
        cover_letter: recruitment.cover_letter ?? "",
        status: recruitment.status,
        position_id: recruitment.position_id ?? undefined,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const method = editRecruitment ? "PATCH" : "POST";
  ...
  const handleDelete = async (recruitment: Recruitment) => {
    if (!confirm(`¿Eliminar al candidato "${recruitment.full_name}"?`)) return;
    ...
    <div className="space-y-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={form.position_id ? "bg-gray-100 cursor-not-allowed" : ""}
                  placeholder={form.position_id ? "Se actualiza automáticamente..." : "Nombre de la posición"}
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, salary_expectation: e.target.value })
                  }
            ...
            <div className="space-y-2">
              <Label htmlFor="position_id">Posición relacionada</Label>
              <Select
      ...
      <Dialog open={pdfViewerOpen} onOpenChange={setPdfViewerOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
          ...
          <div className="flex-1 min-h-150 bg-gray-100 rounded-lg overflow-hidden">
            {selectedPdfUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full p-8">
                    ...
                    <div className="mt-8 p-4 bg-linear-to-br from-[#11B4D9]/10 to-[#038C7F]/5 rounded-lg border border-[#11B4D9]/30 max-w-md">
                      <div className="flex items-start gap-2">
                        <File className="h-5 w-5 text-[#035AA6] mt-0.5 shrink-0" />
                        <div>
