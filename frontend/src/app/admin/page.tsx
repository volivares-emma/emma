  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const responses = await Promise.allSettled([
        ...
        fetch(`/api/assignments?page=1&limit=1`),
        fetch(`/api/certificates?page=1&limit=1`),
        fetch(`/api/blogs?page=1&limit=1`),
        fetch(`/api/slides?page=1&limit=1`),
        fetch(`/api/testimonials?page=1&limit=1`),
        fetch(`/api/notifications?page=1&limit=1`),
        fetch(`/api/job-positions?page=1&limit=1`),
        fetch(`/api/recruitments?page=1&limit=1`),
      ...
      const activeUsers =
        users.data?.filter((user: any) => user.is_active).length || 0;
      const publishedCourses =
        courses.data?.filter((course: any) => course.status === "published")
          .length || 0;
      ...
      const dashboardStats: DashboardStats = {
        users: {
          ...
          growth_rate:
            users.total > 0
              ? Math.round((newUsersThisMonth / users.total) * 100)
              : 0,
        },
        courses: {
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-2">
              <Progress
                value={
                  (stats.courses.published / (stats.courses.total || 1)) * 100
                }
                className="h-1"
              />
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <span className="font-bold">
                  {stats.recruitment.job_positions}
                </span>
                <Button variant="ghost" size="sm" asChild>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <span className="font-bold">
                  {stats.recruitment.recruitments}
                </span>
                <Button variant="ghost" size="sm" asChild>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <span className="font-bold">
                  {stats.recruitment.subscriptions}
                </span>
                <Button variant="ghost" size="sm" asChild>
