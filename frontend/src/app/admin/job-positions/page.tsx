...
export default function JobPositionsPage() {
  ...
  const activePositions = jobPositions.filter((p) => p.is_active).length;
  const featuredPositions = jobPositions.filter((p) => p.is_featured).length;
  const inactivePositions = jobPositions.filter((p) => !p.is_active).length;
  ...
  const openDialog = (position?: JobPosition) => {
    ...
    if (position) {
      setForm({
        ...
        salary_min: position.salary_min === null ? undefined : position.salary_min,
        salary_max: position.salary_max === null ? undefined : position.salary_max,
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]:
        name === "salary_min" || name === "salary_max" || name === "experience_min"
          ? value === ""
            ? undefined
            : parseInt(value)
          : value,
  ...
  const activeJobPositions = jobPositions.filter((p) => p.is_active);
  const inactiveJobPositions = jobPositions.filter((p) => !p.is_active);
                        ...
                        <div className="space-y-3">
                          {position.employment_type && (
                            <Badge
                              className={
                                employmentTypes[
                                  position.employment_type as keyof typeof employmentTypes
                          ...
                          <div className="flex items-center gap-1 text-sm font-medium text-green-700">
                            {formatSalary(
                              position.salary_min === null
                                ? undefined
                                : position.salary_min,
                              position.salary_max === null
                                ? undefined
                                : position.salary_max
                            )}
                          </div>

                          {position.experience_min && (
                            <div className="flex items-center gap-1 text-sm text-muted-foreground">
                              <Target className="h-3 w-3" />
                              Mín. {position.experience_min} años de experiencia
                            </div>
                          )}
                        </div>
