...
export default function Slides() {
  ...
  useEffect(() => {
    if (prefersReducedMotion || statsItems.length <= 1) return;
    const interval = setInterval(() => {
      setStatsIndex((prev) => (prev + 1) % statsItems.length);
    }, 3500);
    return () => clearInterval(interval);
  }, [prefersReducedMotion, statsItems.length]);
  ...
  const fetchFiles = async (slideId: number) => {
    try {
      const response = await fetch(
        `/api/files/public?related_type=slides&related_id=${slideId}`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  const fetchSlides = async () => {
    ...
    try {
      ...
      const plainSlides: Slide[] = Array.isArray(data)
        ? data
  ...
  return (
    <section className="relative">
      <div className="relative flex items-center gap-0.5 overflow-hidden bg-cover bg-center bg-no-repeat transition-all duration-1000 ease-in-out">
        {particlesReady && (
          <Particles
        ...
        <div className="absolute inset-0 overflow-hidden">
          {/* Floating Orbs */}
          <motion.div
            className="absolute top-20 left-10 w-72 h-72 bg-linear-to-r from-[#035AA6]/20 to-[#11B4D9]/20 rounded-full blur-3xl"
            animate={prefersReducedMotion ? {} : { y: [0, -10, 0] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
          />
          <motion.div
            className="absolute bottom-20 right-10 w-96 h-96 bg-linear-to-r from-[#07598C]/20 to-[#038C7F]/20 rounded-full blur-3xl"
          ...
          />
          <motion.div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-linear-to-r from-[#11B4D9]/10 to-[#035AA6]/10 rounded-full blur-3xl"
        ...
        <div className="container md:mb-12 lg:mb-2 min-h-[90vh] mx-auto px-6 md:px-8 lg:px-10 z-10 flex items-center">
          <div className="w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-56">
              {/* Texto */}
              <div className="flex flex-col justify-center md:mt-12 lg:mt-0">
                <MotionFadeUp delay={0.1} key={slide.id}>
                  <h1 className="text-3xl md:text-4xl lg:text-5xl font-black mb-2 leading-tight">
                    <span className="bg-linear-to-r from-white via-cyan-100 to-cyan-200 bg-clip-text text-transparent drop-shadow-2xl">
                      {slide.title}
                    </span>
                  </h1>
                  <p className="text-xl text-white mb-6 font-light leading-relaxed">
                    {slide.subtitle}
                  </p>
                  ...
                  <div className="mt-10 pt-8">
                    <div className="relative overflow-hidden rounded-3xl border border-[#07598C]/60 bg-[#0D0D0D]/30 shadow-[0_0_0_1px_rgba(17,180,217,0.12)]">
                      <div className="absolute inset-y-0 left-0 w-16 bg-linear-to-r from-[#0D0D0D]/85 to-transparent" />
                  ...
                  <div
                    className={`relative rounded-3xl  w-full ${
                      slideVisualType === "image" && hasImage
                        ? "p-0 max-w-2xl bg-transparent"
                        : "p-6 max-w-lg bg-[#0D0D0D]/30"
                    }`}
                  >
                    {slideVisualType === "image" && hasImage ? (
                      <div className="relative w-full overflow-visible rounded-2xl h-90 md:h-100 lg:h-120">
                        ...
                        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                          {/* Ring 1 */}
                          <div
                            className="absolute rounded-full border border-white/12 animate-[spin_28s_linear_infinite]"
                            style={{ width: "58%", height: "58%" }}
                          >
                            <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/10 border border-white/20 backdrop-blur flex items-center justify-center shadow-lg">
                              <Cloud className="h-4 w-4 text-[#11B4D9]" />
                            </div>
                          </div>
                          ...
                          {/* Ring 3 */}
                          <div
                            className="absolute rounded-full border border-white/6 animate-[spin_48s_linear_infinite]"
                            style={{ width: "88%", height: "88%" }}
                          >
                            <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 h-8 w-8 rounded-full bg-white/10 border border-white/20 backdrop-blur flex items-center justify-center shadow-lg">
                              <UserPlus className="h-3.5 w-3.5 text-[#11B4D9]" />
                            </div>
                          </div>

                          {/* Ring 4 (centro) */}
                          <div
                            className="absolute rounded-full border border-white/14 animate-[spin_26s_linear_infinite]"
                            style={{ width: "40%", height: "40%" }}
                          >
                            <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 h-7 w-7 rounded-full bg-white/10 border border-white/20 backdrop-blur flex items-center justify-center shadow-lg">
                              <ShieldCheck className="h-3 w-3 text-[#11B4D9]" />
                            </div>
                          </div>
                          ...
                          {/* Partículas */}
                          <div className="absolute top-10 right-24 h-2 w-2 rounded-full bg-[#11B4D9]/70 shadow-[0_0_12px_rgba(17,180,217,0.6)]" />
                      ...
                      <VisualRenderer type={slideVisualType} />
                    )}
                  </div>
                </MotionFadeScale>
              </div>
